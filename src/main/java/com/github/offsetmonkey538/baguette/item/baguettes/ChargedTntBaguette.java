...
public class ChargedTntBaguette extends Baguette {
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        if (!world.isClient)
            world.createExplosion(user, user.getX(), user.getY(), user.getZ(), ConfigLoader.getConfig().getChargedTntBaguetteExplosionStrength(), Explosion.DestructionType.BREAK);
